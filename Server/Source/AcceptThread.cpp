...
AcceptThread::AcceptThread(DatagramSocket &socket, TPacketStreamBundle &incomingData, TMessageQueue &wakeUpQueue) 
	: Thread("ReceiverThread"), receiveSocket_(socket), incomingData_(incomingData), wakeUpQueue_(wakeUpQueue), blowFish_(RandomNumbers, RandomNumbers_size)
{
	if (!receiveSocket_.bindToPort(serverConnectionPort)) {
